...
- 贯通运营：`throughRunning`（`segments[]` + `joinStationIds[]`，N≥2 段）写入 Generator 状态与 Kyuri naive YAML（`throughRunning` 作为可选字段，仍为 `version: 3`）；反转站序时同步反转分段；删除接续站时将 join 迁到邻站。
- 线路图：段端「线号块 + 渐隐 chevron」、接续站双块、底色条贯通说明文案；启用贯通时运行线与方向箭头相对默认中心下移（对照 6–S1 模板）。
- 方向标：「往」按下一站所属段取线号与该段终点；贯通说明文案与线路图同规格。
- 设置面板弹窗编辑贯通分段与接续站。
...
- 贯通说明文案中每条线均写完整「x号线」（含「乘坐…无需下车换乘」后缀）。
- 「按线路填充站点」现有线网截止时间由 2026.6 改为 **2026.10**。
...
- 按线路标识色合成的 `--theme-*` 调色板：Harmonizer chroma 上限 ×0.75，与 `@umamichi-ui/common-css` 0.20.0 主题饱和度策略对齐（APCA / hue 不变）。
...
- 贴纸导出上的生成器水印（此前未勾选署名承诺时仍可下载并叠加重复水印）。
...
- 拖动线路颜色条卡帧：此前每次 `input` 都更新挂在 `App` 上的草稿状态，使整页（含站点表）重渲染，而 160ms debounce 只延后了预览更新、挡不住渲染；慢速拖动还会中途提交并反复重算预览与主题。
