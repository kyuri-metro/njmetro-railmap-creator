...
- 依赖 `@umamichi-ui/common-components` 升至 ^0.4.0：内容型叠层改用 `FullscreenOverlay` / 更新后的 `ConfirmDialogOverlay`（移动端全屏滑入 + 返回，桌面端居中对话框 + 关闭；叠层打开时锁定滚动条并保留滚动条槽位）。
- 站点编辑 / 设置等对话框改为向壳组件传 `title` 与正文，避免移动端全屏下表单间距被拉散；设置表单在窄屏拉满宽度。

### Fixed

- 全屏浮层毛玻璃挂在 portal 根节点，窄屏打开后再拉宽到桌面断点时四周可正常露出毛玻璃。
